...
- Microsoft Endpoint Configuration Manager (MECM/SCCM) client occasionally invokes encoded PowerShell from `ccmexec.exe` for compliance scripts → candidate parent-process allowlist entry in `lookups/allowlist_powershell_encoded.csv`
...
- `lookups/allowlist_powershell_encoded.csv` records candidate parent-process
  tuning; it is not invoked by the deployed saved search yet.
- Suppression: 30 minutes per `(dest, process_guid)` to avoid duplicates from short-lived re-execution

## Validation

- Atomic Red Team candidate: T1059.001 #2 — Mimikatz via encoded PowerShell
- Promoted evidence method: controlled manual reproduction below
